...
#        - compute MAPE@10 on test set
#        - make it work on new captions (to showcase)
#        - look at efficiency and fine-tuning
...
for epoch in range(epochs):
    print(epoch)
    # 1) loop for images (X)
    for b in range(floor(nr_images / N_x)):
        # Randomize indices
        ...
        dj_df_T = np.zeros(C).reshape(C, 1)
        ...
        for index_mb, i in enumerate(T):
            ...
            dj_df_D = np.zeros(C).reshape(C, 1)
    ...
    for b in range(floor(nr_captions / N_y)):
        # Randomize indices
        ...
        dj_dg_T = np.zeros(C).reshape(C, 1)
